...
def question_structure_analysis_node(state: WorkflowState) -> WorkflowState:
    ...
    If the question starts with 'How many', you MUST respond with 'structured'.
    ...
    # Override logic: If question starts with "how many", force it to be structured (except memory questions)
    user_input_lower = user_input.lower().strip()
    if user_input_lower.startswith("how many"):
        # Check if it's a memory question (these should keep their LLM-determined structure_type)
        memory_keywords = ["before", "earlier", "previous", "last time", "you said", "we discussed", 
                          "from before", "what did you tell me", "you mentioned", "from our conversation"]
        is_memory_question = any(keyword in user_input_lower for keyword in memory_keywords)
        
        if not is_memory_question:
            # Force all non-memory "how many" questions to be structured
...
def structured_processing_node(state: WorkflowState) -> WorkflowState:
    ...
    system_prompt = (
        ...
        "\n"
        "IMPORTANT: When providing your final answer, do NOT mention the specific tool names or technical details.\n"
        "Give natural, user-friendly responses as if you're a helpful data analyst.\n"
        "Do NOT say things like 'use select_semantic_category' or mention internal tool names.\n"
        "\n"
        ...
        "3. For 'how many' questions, follow these rules:\n"
        "   - If asking about something that is a category (e.g., 'how many refund requests', 'how many delivery issues') → Use count_category()\n"
        "   - If asking about something that is an intent (e.g., 'how many people asked for refunds', 'how many want to track orders') → Use count_intent()\n"
        "   - For complex questions like 'how many orders that are not cancellations':\n"
        "     Step 1: Call count_category('ORDER') to get total ORDER count\n"
        "     Step 2: Call count_intent('cancel_order') to get cancel_order count\n"
        "     Step 3: Use sum_numbers(order_count, -cancel_order_count) to subtract\n"
        "   - For questions like 'how many get refund requests and how many order tracking requests':\n"
        "     Step 1: Call count_intent('get_refund') to get get_refund count\n"
        "     Step 2: Call count_intent('track_order') to get track_order count\n"
        "     Step 3: Use sum_numbers(get_refund_count, track_order_count) to add them\n"
        "   - Avoid select_semantic_category() and select_semantic_intent() for simple counting questions - use count_category() and count_intent() instead\n"
        "4. Extract the specific data you need from the memory result (e.g., numbers, categories).\n"
        "5. Use other tools as needed (e.g., sum_numbers for calculations).\n"
        "6. Synthesize the results into a clear answer.\n"
        "7. When you have the final answer, call the finish tool.\n"
        "\n"
        "Examples:\n"
        ...
        "- For 'How many refund requests?': Use count_category('REFUND') for category-based questions\n"
        "- For 'How many people asked to get refunds?': Use count_intent('get_refund') for intent-based questions (NOT select_semantic_intent)\n"
        "- For 'How many delivery issues?': Use count_category('DELIVERY') for category-based questions (NOT select_semantic_category)\n"
        "- For 'How many people want to track their orders?': Use count_intent('track_order') for intent-based questions (NOT select_semantic_intent)\n"
        "- For 'How many orders that are not cancellations?':\n"
        "  Step 1: Call count_category('ORDER') to get total ORDER count\n"
        "  Step 2: Call count_intent('cancel_order') to get cancel_order count\n"
        "  Step 3: Call sum_numbers(order_count, -cancel_order_count) to subtract\n"
        "- For 'How many get refund requests and how many order tracking requests?':\n"
        "  Step 1: Call count_intent('get_refund') to get get_refund count\n"
        "  Step 2: Call count_intent('track_order') to get track_order count\n"
        "  Step 3: Call sum_numbers(get_refund_count, track_order_count) to add them\n"
        "\n"
...
def unstructured_processing_node(state: WorkflowState) -> WorkflowState:
    ...
    allowed_tools = ["select_semantic_intent", "select_semantic_category", "show_examples", "summarize", "show_dataframe", "sum_numbers", "count_category", "count_intent", "memory", "finish"]
    final_response, processing_results, tools_used = react_loop(system_prompt, user_input, allowed_tools, session_history, persistent_context)
...
if __name__ == "__main__":
    ...
    response1, thread_id, _ = run_workflow(test_input1)
    ...
    response2, _, _ = run_workflow(test_input2, thread_id)
